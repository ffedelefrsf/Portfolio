...
- To run package binaries use `pnpm exec <binary>` (replaces `npx`)
- To add shadcn components use `pnpm dlx shadcn@latest add <component>` (replaces `npx shadcn@latest`)
- To install dependencies use `pnpm install` (or `pnpm add <pkg>` / `pnpm add -D <pkg>`)
- To run scripts use `pnpm <script>` (e.g. `pnpm build`, `pnpm dev`)
...
### 5. CSS file discipline

Only add styles to `globals.css` when they are truly global (resets, base typography, design tokens, theme variables).

For page- or component-specific styles:

- Prefer a **CSS Module** (e.g. `styles.module.css`) co-located with the page or component
- Import the module and apply the hashed class to the relevant element — this scopes the rules without polluting the global stylesheet
- A regular non-module `.css` file imported directly into a component is acceptable when scoping via class names is not needed
- Never put page-specific rules in `globals.css`

---

### 6. HTML data attributes

Do not use custom `data-*` attributes unless there is no other way.

Prefer CSS classes (including CSS Module hashed classes) for styling hooks, and React props for passing state or identity between components.
...
This applies to: inline comments, block comments, JSDoc, CSS comments, and explanatory remarks inside any file.

---

### 8. Import sorting

Imports are sorted automatically by ESLint (`eslint-plugin-simple-import-sort`). All imports are kept in a single block — no blank lines between external, internal, or relative imports.

- After making changes, run `pnpm lint:fix` to auto-fix import order
- To check without fixing, run `pnpm lint`
- Never manually reorder imports or add blank lines between import groups

---

### 9. Tailwind CSS usage
...
### 14. SEO and portfolio best practices
...
### 17. Content structure for portfolio pages
...
### 20. File size and separation of concerns
...
### 22. Theme and i18n reminder for every suggestion
...
### 23. Responsiveness and breakpoint support
