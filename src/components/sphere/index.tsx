...
const PhotoSphere = (props: { imageData: string[] }) => {
  ...
  const updateFrame = (mouseShift: number) => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const prevAngle = parseFloat(item.dataset.angle!);
      ...
      const angle = prevAngle + Math.asin(newMouseShift / radius);
      const newXp = Math.sin(angle) * radius;
      const newZp = Math.cos(angle) * radius;
      // let diff;
      // if (prevZp > 0) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      containerRef?.removeEventListener("mousedown", downListener);
    };
  }, [props.imageData]);
